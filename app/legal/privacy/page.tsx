    ...
    <div className="min-h-screen bg-[#F8FAFC]">
      <div className="mx-auto max-w-3xl px-5 py-16 md:px-10">
        ...
        <div className="mb-8">
          ...
          <h1 className="mt-2 text-3xl font-extrabold text-[#001f3f]">{t({ fr: 'Politique de Confidentialité', ht: 'Politik Konfidansyalite' })}</h1>
        ...
        <div className="rounded-2xl border border-[#E2E8F0] bg-white p-8 shadow-sm space-y-6 text-sm leading-7 text-slate-600">
          <section><h2 className="font-bold text-[#001f3f] mb-2">{t({ fr: 'Données que Nous Collectons', ht: 'Done Nou Kolekte' })}</h2><p>Nou kolekte: non, email, done biznis ou antre (vant, acha, depans, pwodui). Nou pa kolekte enfòmasyon finansye pèsonèl (nimewo kont, etc.).</p></section>
          <section><h2 className="font-bold text-[#001f3f] mb-2">{t({ fr: 'Comment Nous Utilisons Vos Données', ht: 'Kijan Nou Itilize Done Ou' })}</h2><p>Done yo sèlman itilize pou ba w sèvis ProfitPilot. Nou pa vann, pataje oswa komèsyalize done ou ak tiye pati.</p></section>
          <section><h2 className="font-bold text-[#001f3f] mb-2">{t({ fr: 'Sécurité', ht: 'Sekirite' })}</h2><p>Tout done yo kriptografye ak Supabase (AES-256). Koneksyon yo pwoteje pa HTTPS. Bakòp otomatik chak jou.</p></section>
