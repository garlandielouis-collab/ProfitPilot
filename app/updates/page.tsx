  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC]">
      <div className="mx-auto max-w-3xl px-5 py-16 md:px-10">
        ...
        <div className="mb-12">
          <span className="text-xs font-bold uppercase tracking-widest text-[#50c878]">{t({ fr: 'Mises à jour', ht: 'Changelog' })}</span>
          <h1 className="mt-2 text-3xl font-extrabold text-[#001f3f]">{t({ fr: 'Dernières nouvelles', ht: 'Dènye Mizajou' })}</h1>
          ...
          {UPDATES.map((u, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}
              ...
              <div className="mb-4 flex items-center gap-3">
                ...
                <span className={`rounded-full px-2.5 py-1 text-[10px] font-bold ${u.color}`}>{t({ fr: { Nouvo: 'Nouveau', Amelyorasyon: 'Amélioration', Korekson: 'Correction', Fonksyon: 'Fonctionnalité', Majè: 'Majeure' }[u.badge] || u.badge, ht: u.badge })}</span>
