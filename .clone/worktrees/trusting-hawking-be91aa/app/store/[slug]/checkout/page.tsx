...
export default function CheckoutPage() {
  ...
  // We need store data — fetch it client-side via effect
  ...
  useState(() => {
    getStoreBySlug(slug).then((s) => {
      ...
      if (s.shipping_modes?.length > 0) {
        setForm((f) => ({ ...f, shipping_mode: (s.shipping_modes as any[])[0]?.id ?? '' }));
      }
    });
  });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      const { orderNumber } = await createStoreOrder({
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-8 lg:grid-cols-2">
        ...
        <div className="space-y-6">
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-6">
            ...
            <div className="space-y-3">
              ...
              <input className={inp} placeholder="Appartement, suite, etc." value={form.line2} onChange={(e) => setForm({ ...form, line2: e.target.value })} />
              ...
              <div className="space-y-2">
                {shippingModes.map((mode) => (
                  <label key={mode.id} className={`flex cursor-pointer items-center justify-between rounded-xl border p-4 transition ${form.shipping_mode === mode.id ? 'border-slate-700 bg-slate-50' : 'border-slate-200 hover:border-slate-300'}`}>
                    ...
                    <span className="text-sm font-bold text-slate-700">{mode.price === 0 ? 'Gratuit' : fmt(mode.price)}</span>
                  </label>
                ))}
              </div>
            </section>
            ...
            <div className="space-y-2">
              {paymentMethods.map((method) => (
                <label key={method} className={`flex cursor-pointer items-center gap-3 rounded-xl border p-4 transition ${form.payment === method ? 'border-slate-700 bg-slate-50' : 'border-slate-200 hover:border-slate-300'}`}>
                  ...
                  <span className="text-sm font-medium text-slate-800">{PAYMENT_LABELS[method] ?? method}</span>
                </label>
              ))}
            </div>
          </section>
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-6">
            ...
            <textarea className={inp} rows={3} placeholder="Instructions spéciales pour votre commande…" value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} />
          </section>
        </div>
        ...
        <div className="space-y-4">
          <div className="rounded-2xl border border-slate-200 bg-white p-6 sticky top-24">
              ...
              {items.map(({ product, quantity }) => {
                ...
                return (
                  <div key={product.id} className="flex items-center gap-3">
                    ...
                    <p className="text-xs font-bold text-slate-800">{fmt(price * quantity)}</p>
                  </div>
                );
              })}
            </div>
            <div className="my-4 border-t border-slate-100 space-y-2 pt-4 text-sm">
            ...
            {error && <p className="mb-3 rounded-xl bg-red-50 border border-red-200 px-4 py-3 text-xs text-red-600">{error}</p>}
            ...
            >
              {submitting
                ...
                : `Confirmer la commande — ${fmt(orderTotal)}`}
            </button>
