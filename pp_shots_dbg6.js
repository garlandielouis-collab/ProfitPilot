...
(async () => {
  ...
  for (const width of WIDTHS) {
    ...
    page.on('request', (r) => { if (r.isNavigationRequest()) console.log('  [req] ' + r.url().slice(-58)); });
    page.on('response', (r) => { if (r.request().isNavigationRequest() || r.status() >= 300) console.log('  [resp] ' + r.status() + ' ' + r.url().slice(-58) + ' -> ' + (r.headers()['location'] || '')); });
