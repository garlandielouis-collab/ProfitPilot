    ...
    <div ref={ref} className="relative px-3 pb-3">
      ...
      >
        ...
        {company.logoUrl ? (
          ...
          <div className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-lg bg-[#001F3F] text-[0.6rem] font-bold text-white dark:bg-[#50C878] dark:text-[#001F3F]">
            {initials}
          </div>
        )}

        <div className="min-w-0 flex-1 text-left">
          <p className="truncate text-xs font-semibold text-[#001F3F] dark:text-white">{company.name}</p>
          {company.sector && (
            <p className="truncate text-[0.6rem] text-slate-400">{company.sector}</p>
          )}
        </div>
      ...
      {open && (allCompanies.length > 1 || canMulti) && (
        <div className="absolute left-3 right-3 top-full z-50 mt-1 rounded-2xl border border-slate-200 bg-white shadow-xl overflow-hidden dark:bg-[#0F172A] dark:border-slate-700">
          <p className="px-3 py-2 text-[10px] uppercase tracking-widest text-slate-400 font-semibold border-b border-slate-100 dark:border-slate-800">
            Mes entreprises
          </p>
          ...
          <div className="max-h-52 overflow-y-auto">
            {allCompanies.map((biz) => (
              ...
              >
                <div className="flex h-6 w-6 items-center justify-center rounded-lg bg-[#001F3F]/10 text-[0.55rem] font-bold text-[#001F3F] dark:bg-white/10 dark:text-white">
                  {biz.name.slice(0, 2).toUpperCase()}
                </div>
                <span className="flex-1 truncate text-left text-xs font-medium">{biz.name}</span>
                {biz.id === company.id && (
                  <Check className="h-3.5 w-3.5 text-[#0047AB]" />
                )}
              </button>
            ))}
          </div>
          ...
          {canMulti && (
            <div className="border-t border-slate-100 dark:border-slate-800">
              ...
              <div className="flex items-center justify-between px-3 py-1.5">
                ...
                {atLimit && (
                  <span className="flex items-center gap-1 rounded-full bg-amber-100 px-2 py-0.5 text-[10px] font-semibold text-amber-700 dark:bg-amber-500/10 dark:text-amber-400">
                ...
                creating ? (
                  <div className="flex items-center gap-2 px-3 py-2">
                    <input
                      ...
                      className="flex-1 rounded-lg border border-slate-200 px-2 py-1 text-xs outline-none focus:border-[#0047AB] dark:bg-white/5 dark:border-slate-700"
                    />
                    <button
                      type="button"
                      onClick={handleCreate}
                      disabled={isPending || !newName.trim()}
                      className="rounded-lg bg-[#0047AB] px-2 py-1 text-xs font-semibold text-white disabled:opacity-50"
                    >
                      OK
                    </button>
                  </div>
                  ...
                  <button
                    ...
                    className="flex w-full items-center gap-2 px-3 py-2.5 text-xs font-medium text-[#0047AB] hover:bg-blue-50 transition dark:hover:bg-blue-500/10"
                  >
