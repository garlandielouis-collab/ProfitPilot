...
export function NewsletterForm({
  ...
  store: StoreView;
  /**
   * `inline` : l'adresse et le bouton côte à côte dès 640 px — la section
   * d'accueil, qui dispose de toute la largeur de la page.
   *
   * `stacked` : l'un sous l'autre, toujours. C'est ce qu'il faut dans la carte
   * du pied de vitrine, qui fait trois colonnes sur douze : en ligne, le
   * bouton y débordait du cadre, et « Je m'inscris » se lisait à moitié.
   * Le point de rupture ne pouvait pas trancher — il mesure la FENÊTRE, et
   * c'est la largeur du CONTENANT qui décide ici.
   */
  ...
  return (
    <form
      ...
      className={`mt-5 flex flex-col gap-2 ${layout === 'inline' ? 'sm:flex-row' : ''}`}
    >
      <input
        type="email"
        required
        ...
        className="min-h-[48px] w-full min-w-0 flex-1 rounded-[8px] border bg-[var(--st-surface)] px-4 text-[15px] text-[var(--st-ink)] outline-none focus:border-[var(--st-accent)]"
